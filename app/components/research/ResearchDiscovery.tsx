...
function CaseColumn({item}:{item:Case}){
  const framing={
    bear:{role:"Downside path",condition:"What must deteriorate",drivers:"Downside drivers"},
    base:{role:"Reference path",condition:"What must remain true",drivers:"Evidence supporting this path"},
    bull:{role:"Upside path",condition:"What must improve",drivers:"Upside drivers"},
  }[item.key as "bear"|"base"|"bull"];
  return <article className={`research-case ${item.key}`}>
    <header><div><span>{framing.role}</span><h3>{item.outcome}</h3><b>{framing.condition}</b></div><small>{item.confidence} confidence</small></header>
    <p>{item.full_text.split(/\n\n/)[0]}</p>
    <div><strong>{framing.drivers}</strong>{item.drivers.length?item.drivers.map(driver=><span key={driver}>• {driver}</span>):<span>More company evidence is needed.</span>}</div>
...
function SecurityOverview({overview,busy,onToggleWatchlist}:{overview:Overview;busy:boolean;onToggleWatchlist:()=>void}){
  ...
  return <div className="unified-security-report">
    ...
    <section className="research-thesis-bridge"><div><span>From evidence to your thesis</span><h3>Start with a draft. Save only what you believe.</h3><p>EagleEyes will prefill a core thesis, assumptions, catalysts, risks, and breakers from verified research. You review and edit every item; nothing becomes your thesis until you explicitly confirm and save it.</p></div><Link className="primary" href={`/decisions?ticker=${encodeURIComponent(overview.ticker)}&source=research`}>Draft {overview.ticker} thesis →</Link></section>
