...
export function DecisionsPage({
  ...
  const scenarioParagraphs=scenarioText.split(/\n\s*\n/).map(item=>item.trim()).filter(Boolean);
  const scenarioHeading=caseTab==="bull"?"Why results could exceed the current path":caseTab==="bear"?"Why results could fall below the current path":`Why the current path could persist`;
    ...
    {!loading&&workspace&&<section className="security-decision-cockpit" aria-label="Security decision dashboard">
      ...
      <div className="security-report">
        ...
        {intelligenceLoading?<section className="panel report-loading"><span/>Loading the stored evidence report…</section>:<>
          ...
          <section className="panel scenario-report"><header><div><span>One-page thesis report</span><h3>Three distinct paths</h3><small>Each case is a short memo explaining a different outcome, why it could happen, and which evidence would confirm or disprove it.</small></div>{drafting&&<span className="scenario-building">Refreshing stored evidence…</span>}</header><div className="scenario-tabs" role="tablist">{(["bear","base","bull"] as const).map(value=><button role="tab" aria-selected={caseTab===value} className={caseTab===value?"active":""} onClick={()=>setCaseTab(value)} key={value}><strong>{value[0].toUpperCase()+value.slice(1)}</strong><small>{value==="bull"?"Better than current path":value==="bear"?"Worse than current path":"Current path persists"}</small></button>)}</div><article className={`scenario-copy ${caseTab}`}><span>{caseTab} case · why</span><h4>{scenarioHeading}</h4><div className="scenario-paragraphs">{scenarioParagraphs.length?scenarioParagraphs.map((paragraph,index)=><p key={`${caseTab}-${index}`}>{paragraph}</p>):<p>The evidence-built case is loading. You can still add or edit your own view below.</p>}</div><section className="scenario-evidence"><strong>{caseTab==="bull"?"Drivers that must strengthen":caseTab==="bear"?"Drivers that could weaken it":"Evidence that must persist"}</strong>{scenarioEvidence.length?scenarioEvidence.slice(0,4).map((item,index)=><small key={`${item}-${index}`}>{item}</small>):<small>No company-specific evidence is stored for this case yet.</small>}</section></article>
