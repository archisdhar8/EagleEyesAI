    ...
    {!loading&&workspace&&<section className="security-decision-cockpit" aria-label="Security decision dashboard">
      ...
      <div className="security-report">
        ...
        {intelligenceLoading?<section className="panel report-loading"><span/>Loading the stored evidence report…</section>:<>
          ...
          <section className="panel scenario-report"><header><div><span>One-page thesis report</span><h3>Three distinct paths</h3><small>Each case explains a different outcome, why it could happen, and which evidence would make it more likely.</small></div>{drafting&&<span className="scenario-building">Refreshing stored evidence…</span>}</header><div className="scenario-tabs" role="tablist">{(["bear","base","bull"] as const).map(value=><button role="tab" aria-selected={caseTab===value} className={caseTab===value?"active":""} onClick={()=>setCaseTab(value)} key={value}><strong>{value[0].toUpperCase()+value.slice(1)}</strong><small>{value==="bull"?"Better than current path":value==="bear"?"Worse than current path":"Current path persists"}</small></button>)}</div><article className={`scenario-copy ${caseTab}`}><span>{caseTab} case · why</span><h4>{scenarioHeading}</h4><p>{scenarioText||"The evidence-built case is loading. You can still add or edit your own view below."}</p><section className="scenario-evidence"><strong>{caseTab==="bull"?"Drivers that must strengthen":caseTab==="bear"?"Drivers that could weaken it":"Evidence that must persist"}</strong>{scenarioEvidence.length?scenarioEvidence.slice(0,4).map((item,index)=><small key={`${item}-${index}`}>{item}</small>):<small>No company-specific evidence is stored for this case yet.</small>}</section></article>
