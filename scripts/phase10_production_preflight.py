...
REQUIRED = (
    ...
    "SUPABASE_PUBLISHABLE_KEY", "CORS_ALLOWED_ORIGINS", "SENTRY_DSN",
    "PRODUCTION_ALERT_DESTINATION", "PRODUCTION_TARGET_ID", "EXPECTED_GIT_REVISION",
    "PRODUCTION_FRONTEND_SERVICE_ID", "PRODUCTION_API_SERVICE_ID",
    "PROVIDER_INGESTION_SERVICE_IDS", "PRODUCTION_BACKUP_ID", "PRODUCTION_BACKUP_VERIFIED_AT",
...
def main() -> int:
    ...
    checks = {f"configured:{name}": configured(name) for name in REQUIRED}
    ...
    checks["declared_pending_are_repo_migrations"] = bool(declared_pending) and all(item in expected for item in declared_pending)
    ...
    print(json.dumps({
        "version": "phase10-controlled-production-preflight-v1",
        "phase": args.phase,
        "ready": not failed,
        "revision": revision,
        "dirty": dirty,
